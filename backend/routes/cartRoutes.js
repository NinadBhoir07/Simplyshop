...
// @route PUT /api/cart
// @desc Update product quantity in the cart for a gueat oe logged-in user
// @acces Public
...
// @desc Remove a prduct from the cart
// @access Public
...
// @route GET /api/cart
// @desc Get logged-n user's or guest user's cart
// @access Public
  ...
  try {
    ...
    if (guestCart) {
      ...
      if (userCart) {
        ...
        // f the user has no existing cart, assign the guset cart to the user
      ...
      res.staus(404).json({ message: "Guest cart not found" });
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server Error" });
