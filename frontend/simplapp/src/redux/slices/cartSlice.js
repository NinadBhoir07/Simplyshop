...
// Helper funciton to save cart to localStorage
...
// Update tje quantity of the item in cart
  ...
  async ({ productId, guestId, userId, size, color }, { rejectWithValue }) => {
    try {
      const response = await axios.delete(
        `${import.meta.env.VITE_BACKEND_URL}/api/cart`, // The URL
        {
          data: { productId, guestId, userId, size, color }, // The Body
        }
      );
      return response.data;
    } catch (error) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addToCart.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload?.message || "Failed to add to the cart";
      })
